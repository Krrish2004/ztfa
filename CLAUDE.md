...
**v1 SNARK simplification (locked, documented):** The HLD's constraint count assumes the circuit operates on full CKKS ciphertexts with RNS limb decomposition into BN254 Fr — non-trivial to express faithfully and pushes prover time to multi-minute on academic-grade hardware. For v1 BTP demo, the circuit operates on a **compressed digest representation**: each ciphertext is summarized by `K=8` BN254 Fr elements derived from a deterministic projection of its serialized form. The client computes `H_i` over this 8-element digest; the aggregator computes the same digest over `c_sum` (we move the `1/N` scalar mult to the **plaintext** side after threshold decryption — equivalent semantics, simpler circuit). This preserves the protocol's structural integrity (commitment binding, additive aggregation in-circuit) at ~8k constraints / <5s proving time. **Security trade-off**: an active aggregator that crafts a `c_sum` whose digest matches the expected sum but whose unobserved bytes are corrupted is not detected by the SNARK alone — it would, however, be caught by the client at decryption (decrypted weights would be garbage; eval-loop sanity check trips). v2 work: scale to full polynomial arithmetic in-circuit (HLD §13.2 STARK upgrade path).
